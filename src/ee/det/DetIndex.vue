...
const slides:Component[]=[
  {
    render(){
      return <Slide title="行列式">
        <ol>
          <li>二阶与三阶行列式</li>
          <li>行列式的性质</li>
          <li>克拉默法则</li>
          <li><Math tex={'n'}/>阶行列式</li>
          <li>对换与逆序数</li>
          <li>行列式按列展开</li>
        </ol>
      </Slide>
  ...
  {
    render(){
      return <Slide title="行列式的性质">
        <p>如何计算行列式？🤔从行列式的<em>性质</em>推导计算公式🤓</p>
        <Math tex={`${DetString([['1', '0'], ['0', '1']])}=1`}/>
        <Math tex={`${DetString([['ka_{11}', 'a_{12}'], ['ka_{21}', 'a_{22}']])}=` +
          `k${DetString([['a_{11}', 'a_{12}'], ['a_{21}', 'a_{22}']])}`}/>
        <Math tex={`${DetString([['a_{1}+b_{1}', 'c_1'], ['a_{2}+b_{2}', 'c_2']])}=` +
  ...
  {
    render(){
      return <Slide title={'行列式的性质2'}>
        <Math tex={`${DetString([['a_1', 'b_1'], ['a_2', 'b_2']])}` +
          `=-${DetString([['b_1', 'a_1'], ['b_2', 'a_2']])}`}/>
        <p>如果<Math tex={'a=kb'}/>，即<Math tex={'a'}/>与<Math tex={'b'}/>平行，那么<Math
  ...
  {
    render(){
      return <Slide title={'利用性质推导'}>
        <Math tex={String.raw`\begin{vmatrix}  a_{11} & a_{12}
      \\ a_{21} & a_{22}
  ...
  },
  {
    render(){
      return <Slide title={'余子式'}>
        <ol>
          <li><Math tex={'a_{43}'}/>的位置移动到<Math tex={'a_{11}'}/>的位置，交换行列<Math tex={'(4-1)+(3-1)=5'}/>次，符号为
            <Math tex={'(-1)^5'}/></li>
          <li>对于一般的<Math tex={'a_{ij}'}/>，需要交换<Math tex={'(i-1)+(j-1)'}/>次行列，符号为
            <Math tex={'(-1)^{(i-1)+(j-1)}'}/>，化简得到<Math tex={'(-1)^{i+j}'}/></li>
          <li><Math tex={'a_{ij}'}/>消去第<Math tex={'i'}/>行第<Math tex={'j'}/>列得到<em>余子式</em>
            ，记为<Math tex={'M_{ij}'}/></li>
          <li>给余子式加上符号就得到<em>代数余子式</em>，记为<Math tex={'A_{ij}'}/>，两者的关系为
            <Math tex={'A_{ij}=(-1)^{i+j}M_{ij}'}/></li>
        </ol>
      </Slide>
    }
  },
  {
    render(){
      return <Slide title={'Laplace定理'}>
        <p>设<Math tex={'A=|a_{ij}|'}/>是<Math tex={'n'}/>阶行列式，那么
  ...
  {
    render(){
      return <Slide title={'逆序数'}>
        <ul>
          <li><Math tex={String.raw`A=\sum_p (-1)^{t(p)} a_{1 p(1)} a_{1 p(2)}\cdots a_{n p(n)}`}/></li>
          <li><Math tex={'p(i)'}/>称为<em>全排列</em></li>
          <li>示例：<table class={styles.perm}>
            <tbody>
            <tr><td><Math tex={'i'}/></td><td>1</td><td>2</td><td>3</td><td>4</td></tr>
            <tr><td><Math tex={'p(i)'}/></td><td>2</td><td>4</td><td>3</td><td>1</td></tr>
            </tbody>
          </table></li>
          <li><Math tex={'t(p)'}/>称为<em>逆序数</em>，决定了展开项的<em>符号</em>，示例中
            <Math tex={'t(p)=4'}/></li>
        </ul>
      </Slide>
    }
