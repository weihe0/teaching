...
const slides:Component[] = [Fourier, FourierDisadvantage, LaplaceTransform,
  {
    render(){
      return <Slide title='卷积示例'>
        <ul>
  ...
  {
    render(){
      return <Slide title='控制具有滞后性'>
        <ConvAnime controls={[-1]} responses={responses}/>
        <p>控制信号瞬间下达，响应<em>持续一段时间</em></p>
      </Slide>
    }
  },
  {
    render(){
      return <Slide title='单位响应'>
        <ConvAnime controls={[1]} responses={responses}/>
  ...
  {
    render(){
      return <Slide title='控制具有时不变性'>
        <ConvAnime controls={[0,0,0,0,0,1]} responses={responses}/>
        <p>如果控制信号延迟<Math tex={String.raw`\tau`}/>时间，
          响应也延迟<Math tex={String.raw`\tau`}/>时间</p>
      </Slide>
    }
  },
  {
    render(){
      return <Slide title='控制与卷积'>
        <ConvAnime controls={[1,1,1,0,-1]} responses={responses}/>
        <p>一段连续的控制信号作用于控制对象上，响应就是控制信号与单位响应的<em>卷积</em></p>
      </Slide>
    }
  },
  {
    render(){
      return <Slide title='卷积的参考资料'>
        <a href='https://www.bilibili.com/video/BV1Sb4y1v7tb?p=6' target='_blank'>卷积与单位脉冲响应</a>
      </Slide>
    }
  },
  ConvLaplace,
