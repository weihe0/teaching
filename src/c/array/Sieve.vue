...
<template>
  <Slide title='Eratosthenes筛除法找素数'>
    <table>
      <tbody>
      <tr class='index'><td  v-for='(e,i) of isPrime' :class='{composite:!e}'>{{i}}</td></tr>
      <tr><td v-for='(e,i) of isPrime'
              :class='{visit:visit[i]}'>{{e ? 1 : 0}}</td> </tr>
      </tbody>
    </table>
    <p>素数：<span v-for='p of prime_numbers' class='prime-number'>{{p}}</span> </p>
  </Slide>
